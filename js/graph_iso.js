...
d3.json("data/graph3.json", function(error, data) {
    nodedata = data.nodes;
});
    ...
    function ticked() {
        ...
        graph.nodes.forEach(function(node) {
            nodedata.forEach(function(d) {
                if (node.id == d.id) {
                    node.check_ins = d.check_ins;
                    // node.xpos = d.xpos;
                    // node.ypos = d.ypos;
                }
            });
            if (node.group == nodekey) {
                valsnodes.push(node.check_ins);
            }
        });
        // console.log(valsnodes);

        var nodesize = d3.scaleLinear()
            .range([3, 20])
            .domain([Math.min(...valsnodes), Math.max(...valsnodes)]);
        // console.log(nodesize.domain());

        node
            .attr("r", function(d) {
                if (d.group != nodekey) return 0;
                return nodesize(d.check_ins);
            })
            .attr("cx", function (d) {
